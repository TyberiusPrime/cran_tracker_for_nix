...
class REcosystem:
    ...
    def dump(self, archive_date, snapshot_date, output_path):
        ...
        (output_path / "excluded_packages.json").write_text(
            json.dumps(excluded_packages_notes, indent=2)
        )
        ...
        (output_path / "header.json").write_text(json.dumps(header, indent=2))

        (output_path / "downgraded_packages.json").write_text(
            json.dumps(
                match_override_keys(
        ...
        test_file = Path(
            output_path / (".packages_tested.ignore" + format_date(archive_date))
        )
        if test_file.exists():
        ...
        (output_path / '.done').write_text("Yes")
    ...
    def run_nix_build(self, output_path, exit_on_failure=True):
...
""".replace(
        ...
        self.run_nix_build(temp_flake_path)
...
def main():
    ...
    already_dumped = [
        fn.name
        for fn in output_toplevel.glob("*")
        if re.match(r"\d{4}-\d{2}-\d{2}", fn.name)
        and (fn / '.done').exists()
    ]
    if len(sys.argv) <= 1:
