...
class REcoSystemDumper:
    ...
    def dump(self, output_path_top_level):
        ...
        job_flake_packages = self.add_packages_to_flake(output_path, job_packages)
    ...
    def add_packages_to_flake(self, output_path, job_packages):
        def gen():
            all_the_packages = set()
            for name, info in self.package_info["all_packages"].items():
                # if name == "robis":
                # raise ValueError(info)
                for k in ["native_build_inputs", "build_inputs"]:
                    for p in info.get(k, []):
            ...
            for p in all_the_packages:
                inputs = [
                    x.relative_to(Path(".").absolute())
                    for x in (flake_auxillaries_path / "packages").glob(p + "*")
                ]
                if not inputs:
                    raise ValueError("CRAN_TRACK_PACKAGE, but no files found", p)
                if not any((x.name == p + ".nix" for x in inputs)):
                    raise ValueError(
                        f"{p}.nix requested by CRAN_TRACK_PACKAGE not found in flake_auxillaries_path / packages"
                    )
        ...
        return ppg2.JobGeneratingJob(
            "add_packages_to_flake_" + format_date(self.archive_date), gen
        ).depends_on(job_packages)
    ...
    def _write_package(self, name, info, op):
        ...
        for (key, arg) in [
            ...
            if key in info:
                ...
                out[arg] = o
        if "patches" in info:
            out["patches"] = [nix_literal("./../patches/" + x) for x in info["patches"]]
    ...
    def dump_cran_packages(self, output_path):
        ...
        def gen(output_path):
            ...
            with tempfile.TemporaryFile("w+") as op:
                ...
                op.seek(0)
                out = op.read()
                out = common.nix_pretty_print(out)
